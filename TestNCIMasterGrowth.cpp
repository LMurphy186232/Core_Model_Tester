...
TEST(NCIMasterGrowth, TestErrorProcessing) {
  ...
  try
  {
    p_oSimManager->ReadFile(WriteNCIGrowthXMLErrorFile1A());
  ...
  //Max. radius of neighbor effects must be > 0
    try
    {
      p_oSimManager->ReadFile(WriteNCIGrowthXMLErrorFile1B());
 ...
 try
 {
   p_oSimManager->ReadFile(WriteNCIGrowthXMLErrorFile14A());
 ...
 try
  {
    p_oSimManager->ReadFile(WriteNCIGrowthXMLErrorFile14B());
    FAIL() << "TestNCIMasterGrowth error processing didn't catch error for WriteNCIGrowthXMLErrorFile14.";
  }
  catch (modelErr& err)
  {
